...
export async function POST(request: Request) {
  ...
  const result = await supabase.from('partners').insert({ name: data.name, short_name: data.shortName, descriptor: data.descriptor, website: data.website || null, logo_path: data.logoPath || null, sort_order: data.sortOrder, is_active: data.isActive }).select('id').single();
