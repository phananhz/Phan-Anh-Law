...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (data.displayRow !== undefined) update.display_row = data.displayRow;
  if (data.isActive !== undefined) update.is_active = data.isActive;
  const supabase = await createSupabaseServerClient();
  if (!supabase) return NextResponse.json({ error: 'Supabase chưa được cấu hình.' }, { status: 503 });
  const result = await supabase.from('partners').update(update).eq('id', id).select('id, is_active, display_row').maybeSingle();
  ...
  return NextResponse.json({ ok: true, id: result.data.id, isActive: result.data.is_active, displayRow: result.data.display_row });
}
