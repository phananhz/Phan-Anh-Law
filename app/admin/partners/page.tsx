...
type PartnerRow = { id: string; name: string; short_name: string; descriptor: string; website: string | null; logo_path: string | null; sort_order: number; is_active: boolean };
...
export default async function AdminPartnersPage({ searchParams }: { searchParams: Promise<{ q?: string; active?: string; page?: string }> }) {
  ...
  if (supabase) {
    let request = supabase.from('partners').select('id, name, short_name, descriptor, website, logo_path, sort_order, is_active', { count: 'exact' });
    if (queryText) request = request.ilike('name', '%' + queryText + '%');
    if (active) request = request.eq('is_active', active === 'true');
    const result = await request.order('sort_order', { ascending: true }).order('name', { ascending: true }).range(from, from + pageSize - 1);
    ...
    rows = filtered.map((partner) => ({ id: partner.id, name: partner.name, short_name: partner.shortName, descriptor: partner.descriptor, website: partner.website || null, logo_path: partner.logoUrl || null, sort_order: partner.sortOrder || 0, is_active: partner.isActive !== false })).slice(from, from + pageSize);
    total = rows.length;
  }
