...
export default async function AdminMessagesPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; page?: string }> }) {
  ...
  return <div className="px-6 py-10 sm:px-10 lg:px-14 lg:py-14">
    ...
    <div className="mt-6 overflow-hidden rounded-2xl border border-stone-200 bg-white/80 shadow-sm">
      {messages.length ? <div className="divide-y divide-stone-100">{messages.map((message) => <div key={message.id} className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex min-w-0 items-start gap-3"><div className="mt-0.5 rounded-xl bg-emerald-50 p-2 text-emerald-brand"><MessageSquare className="h-4 w-4" /></div><div className="min-w-0"><div className="truncate text-sm font-semibold text-stone-900">{message.full_name} · {message.company}</div><div className="mt-1 truncate text-xs text-stone-500">{message.email} · {message.practice}</div><div className="mt-1 text-xs text-stone-400">{message.phone}</div></div></div>
        <div className="flex items-center gap-4 text-xs text-stone-500"><span>{new Date(message.created_at).toLocaleDateString('vi-VN')}</span><span className="rounded-full bg-stone-100 px-3 py-1">{statusLabel(message.status)}</span><Link href={'/admin/messages/' + message.id} aria-label={'Xem tin nhắn của ' + message.full_name} className="rounded-full p-2 transition hover:bg-stone-100"><Eye className="h-4 w-4" /></Link></div>
      </div>)}</div> : <div className="p-12 text-center text-sm text-stone-500">{queryText || status ? 'Không có tin nhắn phù hợp.' : 'Chưa có tin nhắn. Khi form liên hệ được gửi, yêu cầu sẽ xuất hiện tại đây.'}</div>}
    </div>
