...
export default async function AdminNewsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; page?: string }> }) {
  ...
  return <div className="px-6 py-10 sm:px-10 lg:px-14 lg:py-14">
    ...
    <div className="mt-6 overflow-hidden rounded-2xl border border-stone-200 bg-white/80 shadow-sm">{articles.length ? <div className="divide-y divide-stone-100">{articles.map((article) => <NewsActions key={article.id} id={article.id} title={article.title} slug={article.slug} status={article.status} updatedAt={article.updated_at} canDelete={admin?.role === 'super_admin'} />)}</div> : <div className="p-12 text-center text-sm text-stone-500">{queryText || status ? 'Không có bài viết phù hợp.' : 'Chưa có bài viết trong Supabase. Hãy tạo bài đầu tiên để bài xuất hiện trên website.'}</div>}</div>
