...
type PartnerDraft = { id?: string; name: string; shortName: string; descriptor: string; website: string; logoPath: string; sortOrder: number; isActive: boolean };

export default function PartnerEditor({ initialPartner }: { initialPartner?: PartnerDraft }) {
  const router = useRouter();
  const [form, setForm] = useState<PartnerDraft>(initialPartner || { name: '', shortName: '', descriptor: '', website: '', logoPath: '', sortOrder: 0, isActive: true });
  ...
  return <form onSubmit={(event) => void save(event)} className="grid gap-6 lg:grid-cols-[1fr_320px]">
    ...
    <aside className="h-fit space-y-5 rounded-2xl border border-stone-200 bg-white/80 p-6 shadow-sm lg:sticky lg:top-6">
      <label className="block text-sm font-semibold text-stone-800">Thứ tự hiển thị<input type="number" min={-100000} max={100000} step={1} value={form.sortOrder} onChange={(event) => update('sortOrder', Number(event.target.value))} className="mt-2 w-full rounded-xl border border-stone-200 bg-white px-3 py-3 text-sm outline-none focus:border-emerald-brand" /></label>
